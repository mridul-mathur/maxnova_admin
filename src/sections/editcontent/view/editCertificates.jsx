...
import {
  ...
  getCertificates,
  deleteCertificate,
} from "src/redux/actions/utilsAction";

export default function UpdateCertificates() {
  const [state, setState] = useState(null);
  const dispatch = useDispatch();
  const certificates = useSelector(
    (reduxState) => reduxState.utils.certificates
  );
  ...
  const handleAdd = () => {
    ...
    const fields = ["text", "desc", "image_alt"];

    fields.forEach((field) => {
      formData.append(field, certificate[field]);
    });
